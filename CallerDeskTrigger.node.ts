...
export class CallerDeskTrigger implements INodeType {
    ...
    async webhook(this: IWebhookFunctions): Promise<IWebhookResponseData> {
        ...
        try {
            ...
            const statusFilter = this.getNodeParameter('statusFilter', 0) as string;
            const requireSource = this.getNodeParameter('requireSource', 0) as boolean;
            const normalizeOutput = this.getNodeParameter('normalizeOutput', 0) as boolean;
            ...
            const incomingAccount =
                rawData.account_id ?? rawData.accountId ?? '';
            ...
            const incomingStatus = String(
                rawData.Status || rawData.callstatus || ''
            ).toUpperCase();
            ...
            let output: IDataObject = rawData;
